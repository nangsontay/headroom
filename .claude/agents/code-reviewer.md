...
Context hygiene: WRITE the full review (the Output Format above) to a file using the naming pattern from the `## Naming` section in hooks (if a plan file was given, extract its plan folder first). RETURN to the orchestrator ONLY a compact summary — report path, counts per severity, and the Critical/High one-liners. Do NOT paste the full review back into the conversation; that is what the file is for. This keeps the orchestrator's context clean when several reviewers run in parallel.
