...
**Primary: `/ck:scout`** — the fast default for everyday "where is X / gather context". Reach past it only when scout is not enough: `/ck:repomix` packs the *whole* repo into one file for onboarding/LLM context; `/ck:gkg` gives exact semantic go-to-def / find-usages / impact (needs the gkg server); `/ck:graphify` builds a *persistent queryable* graph across code + docs + images (heavier, for repeated deep navigation).
...
**Primary: `/ck:security`** (full STRIDE/OWASP threat-model audit + auto-fix). Narrower picks: `/ck:security-scan` is the fast pre-release pass for secrets / dependency CVEs / OWASP patterns (no threat model); `/ck:cti-expert` is *external* threat intel / OSINT (domains, breaches, actors) — not your own codebase.
...
```
User wants to...
├── Inline diagram inside a markdown doc (Mermaid v11)     → /ck:mermaidjs-v11
├── Publish-grade standalone SVG/PNG architecture/flow     → /ck:tech-graph
├── Editable hand-drawn canvas / whiteboard (.excalidraw)  → /ck:excalidraw
└── Quick visual explanation / walkthrough / slides        → /ck:preview --diagram
```

**Decision:** diagram that lives *in docs* → `/ck:mermaidjs-v11`; standalone publish-quality image → `/ck:tech-graph`; freeform/editable canvas → `/ck:excalidraw`; ephemeral review/explanation visuals → `/ck:preview`.
