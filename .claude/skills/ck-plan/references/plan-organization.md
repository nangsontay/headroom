...
## Optional: kit-local derive engine (no external CLI)

When the `ck` CLI is unavailable, or you want plan files that never drift, use
the kit-local decision-log engine. The append-only `decisions.log.jsonl` is the
single source of truth; `plan.md` and `phase-XX-*.md` are GENERATED from it, so
hand-edits never silently diverge (the `plan-derive-guard` PostToolUse hook warns
if you edit a generated file). Pure Node, deterministic, no Python.

```bash
# Append decisions (each prints a stable id, e.g. D3 (AD-1)):
node .claude/scripts/plan-log-append.cjs <plan-dir> '{"kind":"meta","title":"...","strategy":"..."}'
node .claude/scripts/plan-log-append.cjs <plan-dir> '{"kind":"phase","phase":1,"title":"...","priority":"P1","tranche":"A","deps":[],"overview":"..."}'
node .claude/scripts/plan-log-append.cjs <plan-dir> '{"kind":"decision","phase":1,"title":"...","body":"...","supersedes":"AD-2"}'
node .claude/scripts/plan-log-append.cjs <plan-dir> '{"kind":"task","phase":1,"text":"..."}'

# Regenerate plan.md + phase files (run after each append batch):
node .claude/scripts/plan-derive.cjs <plan-dir>
```

To change a generated plan: append (don't edit), then re-derive. Supersede a stale
decision with a new entry carrying `supersedes: "<AD-id>"`. Entry schema:
`claude/schemas/decision-log.schema.json`. This is opt-in and orthogonal to the
`ck plan` CLI above — plans not created this way are untouched by the guard hook.
