...
<PLAN-HANDOFF>
When the plan directory contains `decisions.log.jsonl` (the derive-engine canonical log), READ it and seed the requirements gate with its entries — cite `AD-{n}`/`D{n}` ids in your scope summary instead of re-eliciting decisions the plan already recorded.

This handoff is **advisory by default**: it makes the HARD-GATEs cheaper/targeted, it does NOT replace them. Scout-first and exact-requirements still run; you re-verify against live code, just faster because the upstream decisions are known.

`--trust-upstream` (default OFF): only for a LEAN-tier derived phase whose decisions are already concrete and low-risk — lets cook treat the recorded decisions as satisfying the requirements gate and skip re-scouting. Never combine with high-risk touchpoints (auth, payments, migrations, public API, concurrency). When used, state it explicitly so the user sees the gate was intentionally short-circuited.

Generated plan/phase files are derived output — do NOT hand-edit them; change the plan by appending to `decisions.log.jsonl` and re-deriving (`plan-derive.cjs`).
</PLAN-HANDOFF>
...
## Effort Tiers (proportionate effort)

Classify the change BEFORE implementing, then scale ceremony to risk. State the chosen tier and why. Bias UP a tier whenever in doubt.

| Tier | Trigger (size AND risk) | Mandatory subagents | Gates |
|------|-------------------------|---------------------|-------|
| **lean** | ≤2 files, no public-contract/schema/security touch, easily reversible (docs, copy, config, isolated bugfix) | none — implement + self-verify (run available tests/build) | code-review optional; finalize sync-back still runs |
| **standard** | typical feature/refactor, ~3–10 files, no high-risk surface | `code-reviewer` + `tester` | full Blocking Gates |
| **full** | high blast radius (auth, payments, migrations, public API, concurrency), >10 files, or irreversible | `code-reviewer` + `tester` + `/ck:project-management` (+ `docs-manager`, `git-manager` at finalize) | full Blocking Gates + HARD-GATE-NO-SIDE-EFFECTS walked explicitly |

**Path-sensitivity override:** any touch to auth, payments, DB migrations, public API, or concurrency forces tier ≥ **standard**, regardless of size.

**Misclassifying DOWN is the failure mode** — when unsure between two tiers, pick the higher one.

## Required Subagents (tier-scoped)
...
| Testing | `tester`, `debugger` | **MUST** spawn at standard/full |
| Review | `code-reviewer` | **MUST** spawn at standard/full |
| Finalize | `/ck:project-management` skill + `docs-manager`, `git-manager` subagents | **MUST** invoke at full; project-management sync-back runs at every tier |

**CRITICAL ENFORCEMENT (tier-aware):**
- **standard/full:** testing, review, and finalization **MUST** be DELEGATED via the Task tool — ending with 0 Task tool calls is INCOMPLETE.
- **lean:** subagents are optional; you MAY implement + verify inline, but you MUST still (a) state the tier + rationale, (b) run available tests/build, and (c) do the finalize plan sync-back. Never skip review on a change that is actually standard/full.
- DO NOT implement standard/full testing, review, or finalization yourself — DELEGATE.
