...
3. **Each persona analyzes independently** — fresh takes, no cross-influence during this phase
4. **Identify agreements** — points where all (or 4+) personas align
5. **Force a dissenter** — premature consensus is a failure mode. If no persona objects, the Devil's Advocate MUST mount the strongest real case against the proposal before any verdict. Surface the conflict; do not smooth it over.
6. **Weigh tradeoffs** — for each conflict, evaluate which concern has higher impact. **Never rewrite what a persona argued** to ease resolution — preserve each view verbatim; the verdict resolves *over* the disagreement, it does not erase it.
