...
/**
 * Build decision-log pointer when the active plan uses the derive engine (~30 tokens).
 * Surfaces the canonical log so agents cite D{n}/AD-{m} ids and never hand-edit
 * generated plan files. Cheap pointer only — agents read the log on demand.
 */
function buildDecisionLogSection(activePlan) {
  if (!activePlan) return [];
  try {
    const dir = activePlan.endsWith('.md') ? path.dirname(activePlan) : activePlan;
    const logPath = path.join(dir, 'decisions.log.jsonl');
    if (!fs.existsSync(logPath)) return [];
    return [
      ``,
      `## Plan Decision Log (canonical)`,
      `- ${logPath}`,
      `- plan.md/phase files are GENERATED from it — cite D{n}/AD-{m} ids; never hand-edit generated files (append + re-derive).`,
    ];
  } catch (_) {
    return [];
  }
...
async function main() {
  ...
  try {
    ...
    // Decision-log pointer (only when the active plan uses the derive engine)
    lines.push(...buildDecisionLogSection(activePlan));
    lines.push(``);
